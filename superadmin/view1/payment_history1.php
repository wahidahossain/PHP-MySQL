...
<th><?php echo $due;?></th>
            ...
            <table class="table table-striped table-bordered table-hover">
                ...
                //new_customer_id 	customer_id 	customer_name 	customer_details 	opening_balance 	status 	customer_category_id 	customer_level_id 	create_date 	update_date
                include ("model/connect.php");
$result_cha = mysql_query("SELECT * FROM `project_sale` WHERE `id`='$id' and `channel_entry_id`='$channel_entry_id' ORDER BY `project_sale`.`date` ASC");
                $count = 0;
                
 while ($row_cha = mysql_fetch_array($result_cha))
                {
                    $project_sale_id00 = $row_cha['project_sale_id'];
                $result78 = mysql_query("SELECT * FROM `payment` WHERE `pre_budget_form_step1_id`= '$project_sale_id00'");
                $row78 = mysql_fetch_array($result78);
                    
                    $count = $count + 1;
                    $payment_id = $row78['payment_id'];
                    $payment = $row78['payment'];
                    $type = $row78['type'];
                    $voucher_no = $row78['voucher_no'];
                    $received_by = $row78['received_by'];
                    $paid_by = $row78['paid_by'];
                    $note = $row78['note'];
                    $date = $row78['date'];
                    ?>
